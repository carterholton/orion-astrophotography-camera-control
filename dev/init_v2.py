...
dbase = {"Target":"empty", "EL":0, "IN":0, "TF":0, "iso_key":5, "Taken":0}
...
def load_project():
    ...
    with open(project, 'r') as file:
        ...
        shots_taken = (lines[5].split(': ',1)[1]).strip()

        dbase = {"Target":target, "EL":int(EL), "IN":int(IN), "TF":int(TF), "iso_key":int(iso_key), "Taken":int(shots_taken)}
        cam_config()
...
def cam_config():
    ...
    if True:
        ...
        lcd.text("TARGET: " + dbase["Target"] + " (" + (str(round((dbase["Taken"]/TF)*100))) + "%)", 1)
        percent = (round((dbase["Taken"]/TF)*100))
        Flag = False
        first = 1
        ...
        while not Flag:
            ...
            if (key == '7' or '4' or '8' or '5' or '9' or '6' or '-' or '+' or '1' or '2' or '3' or 'q' or 'enter' or 'backspace') or (first == 1):
                if key == '7':
                    EL = (EL + 2)
                ...
                if first == 1:
                    first = 0
                ISO = iso_index[iso_key]
                lcd.text(dbase["Target"] + "("+ str(percent) + "->" + (str(percent + round((TF/dbase["TF"])*100))) + "%)", 1)
                lcd.text("EL:" + str(EL) + "s IN:" + str(IN) + "s TF:" + str(TF), 2)
                lcd.text("Av: " + "___" + " ISO:" + str(ISO), 3)
                est_time = round(((EL+IN) * TF) / 60)
                est_expo = round((EL * TF) / 60)
